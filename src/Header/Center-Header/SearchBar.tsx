  ...
  const items: MenuProps["items"] = [
    {
      label: (
        <div className="flex items-center cursor-auto justify-between p-2 rounded">
          ...
          <button onClick={() => setVisible(false)} className="border-0 cursor-pointer">
            ...
            {["All", "Teams", "Leagues", "Players"].map((item, index) => (
              <button
                ...
                className={`rounded-[8px] mobile-menu-p-sizedown py-[8px] text-[12px] w-fill cursor-pointer font-semibold 
                  ...
                  ${selectedTab === item ? "bg-[#7F3FFC] text-white" : "text-[#23262E]/70 "}
                `}
    ...
    { type: "divider" },
    {
      label: <div className="flex items-center p-2 hover:bg-gray-100 rounded">Teams</div>,
      key: "0",
    },
  ...
  return (
    <form className="relative h-10 flex " onSubmit={(e) => e.preventDefault()}>
      ...
      >
        <button
          type="submit"
          onClick={() =>window.innerWidth < 1024? setVisible(true):visible}
