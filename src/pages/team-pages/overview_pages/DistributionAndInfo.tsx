...
export default function DistributionAndInfo() {
  ...
  return (
    <div className="w-full grid grid-cols-[1fr_1fr] max-lg:block  gap-5">
      <div className="grid-1 rounded-[12px] px-5 gap-3 flex flex-col w-full max-lg:hidden bg-[var(--color-bg)]">
          ...
          <div className="grid-1 items-center flex justify-between">
            ...
            <div className="flex gap-2">
              {[7, 2, 1, 0, 1, 6, 1].map((item, index) => (
                <div
                  key={index}
                  className={`w-6 h-6 ${GetGreenColor(
                    index
        ...
        <p className="text-xs text-[var(--color-text-light)]">
          *English Premier League Goal distribution from 4 Matches
        </p>
      </div>
